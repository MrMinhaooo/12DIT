...
# valid option for each snack <full name, letter code ( a - e )
...
if check_snack == "Yes":
    ...
    while desire_snack != "xxx":
        ...
        # if item has a number, separate it into two (number / choice)
        if re.match(number_regex, desire_snack):
            amount = int(desire_snack[0])
            desire_snack = desire_snack[1:]

        else:
            amount = 1
            desire_snack = desire_snack

        # remove white space around snack
        desire_snack = desire_snack.strip()
